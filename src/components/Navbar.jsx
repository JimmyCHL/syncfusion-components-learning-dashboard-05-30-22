...
const Navbar = () => {
  ...
  useEffect(() => {
    if (screenSize <= 900) {
      setActiveMenu(false); // alway close when screen size change
    } else {
      if (activeMenu) {
        // on bigger screen depend on user decision
        setActiveMenu(true);
      } else {
        setActiveMenu(false);
      }
  ...
  return (
    <div className="flex justify-between p-2 md:mx-6 relative">
      ...
      <div className="flex">
        <NavButton title="Cart" customFunc={() => handleClick('chat')} color={currentColor} icon={<FiShoppingCart />} />
        <NavButton title="Chat" dotColor="#03C9D7" customFunc={() => handleClick('cart')} color={currentColor} icon={<BsChatLeft />} />
        ...
        <TooltipComponent content="Profile" position="BottomCenter">
          <div className="flex items-center gap-2 cursor-pointer p-1 hover:bg-light-gray rounded-lg" onClick={() => handleClick('userProfile')}>
            ...
            <p>
              ...
              <span className="text-gray-400 font-bold ml-1 text-14">Jimmy</span>
            </p>
