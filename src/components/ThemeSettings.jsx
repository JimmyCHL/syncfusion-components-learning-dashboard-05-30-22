...
const ThemeSettings = () => {
  ...
  return (
    <div className="bg-half-transparent w-screen fixed nav-item inset-0">
      <div className="float-right h-screen dark:text-gray-200 bg-white dark:[#484B52] w-400">
        <div className="flex justify-between items-center p-4 ml-4">
          <p className="font-smeibold text-xl">Settings</p>
          <button
            type="button"
            onClick={() => {
            ...
            {themeColors.map((item, index) => (
              <TooltipComponent key={index} content={item.name} position="TopCenter">
                <div className="relative mt-2 cursor-pointer flex gap-5 items-center">
                  <button type="button" className="h-10 w-10 rounded-full cursor-pointer" onClick={() => setColor(item.color)}>
                    <BsCheck
